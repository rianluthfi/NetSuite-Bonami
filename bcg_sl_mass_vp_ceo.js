    ...
    function(serverWidget, search, log, record, http, url, redirect) {
			...
			if (context.request.method === 'GET') {
				...
				for (var i = 0; i < lineCount; i++){
					...
					if (checkValue === 'T'){
						...
						masterVP.setValue('custbody_bcg_approved_level_3', true);
						masterVP.save();
					}
				}
				
				redirect.redirect({
					// redirect ke Mass Approve Vendor Payment by CEO
					url: '/app/site/hosting/scriptlet.nl?script=207&deploy=1',
		...
		function renderList(results){
			...
			var form = serverWidget.createForm({
				title : 'Mass Approve Vendor Payment by CEO'
			});
		...
		function findPO(){
			...
			// Saved Search ID Vendor Payment to Approve (CEO)
