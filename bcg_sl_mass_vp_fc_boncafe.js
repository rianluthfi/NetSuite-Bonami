    ...
    function(serverWidget, search, log, record, http, url, redirect) {
        function onRequest(context) {
			...
			if (context.request.method === 'GET') {
				...
				redirect.redirect({
					// redirect ke Mass Approve Vendor Payment by FC (Boncafe)
					url: '/app/site/hosting/scriptlet.nl?script=206&deploy=1',
		...
		function renderList(results){
			...
			var form = serverWidget.createForm({
				title : 'Mass Approve Vendor Payment by FC (Boncafe)'
			});
		...
		function findPO(){
			...
			// Saved Search ID Vendor Payment to Approve (FC) Bu Tati
