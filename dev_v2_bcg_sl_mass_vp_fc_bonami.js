    ...
    function(serverWidget, search, log, record, http, url, redirect) {
        function onRequest(context) {
			...
			if (context.request.method === 'GET') {
				...
				context.response.write('This Write !');
				...
				for (var i = 0; i < lineCount; i++){
					...
					if (checkValue === 'T'){
						...
						masterVP.setValue('custbody_bcg_approved_level_2', true);
						masterVP.save();
					}
				}
				
				redirect.redirect({
					// redirect ke Mass Approve Vendor Payment by FC (Bonami)
					url: '/app/site/hosting/scriptlet.nl?script=208&deploy=1',
		...
		function renderList(context){
			...
			log.debug('START RenderList');
			var form = serverWidget.createForm({
				title : '[DEV] Mass Approve Vendor Payment by FC (Bonami)'
			});
			
			var vendorFilter = form.addField({
				id : 'custpage_vendorfilter',
			...
			// var price = context.request.parameters.custpage_vendorfilter;
			// log.debug('price '+price);
			
			var results = translate(findPO());
			...
			log.debug('END RenderList');
			return form;
		}
		
		function findPO(){
			log.debug('START findPO');
			// Saved Search ID Vendor Payment to Approve (FC) Bonami
